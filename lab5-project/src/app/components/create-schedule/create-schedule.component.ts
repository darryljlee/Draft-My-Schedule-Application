...
//createSchedule functon that's called when user clicks on "create schedule button"
  createSchedule():void{
    ...
    this.http.post<any>("/api/schedules/createaschedule?name="+nameInput, nameInput).subscribe(data =>{ //make a post request to create a schedule when user enters Create Schedule
  ...
  addCourseToSchedule(subject:String, courseNumber:String){ //function that's called when the [+] button is clicked next to a course
    ...
    this.http.put<any>("/api/schedules/updateCourse?nameSched="+nameOfSchedule+ "&subject="+subject+"&courseNumber="+courseNumber, nameOfSchedule).subscribe(data =>{
  ...
  submitResults():void{
    ...
    var submitRequest = "/api/courses/submit?" +"Subject=" + subjectChoice + "&CourseNumber=" + numberChoice + "&Component=" + componentChoice; 
  ...
  ngOnInit(): void {
    ...
    this.http.get<any>("/api/courses").subscribe(data => { 
    ...
    this.http.get<any>("/api/schedules/getallschedules").subscribe(data =>{ 
